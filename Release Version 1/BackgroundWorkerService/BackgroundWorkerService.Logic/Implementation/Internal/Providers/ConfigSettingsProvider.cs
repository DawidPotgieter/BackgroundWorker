...
namespace BackgroundWorkerService.Logic.Implementation.Internal.Providers.Configuration
{
	...
	/// </summary>
	class ConfigSettingsProvider : System.Configuration.ConfigurationSection, ISettingsProvider, ILoggingSettingsProvider
	{
		...
		/// <summary>
		/// This value is used if <see cref="BackgroundWorkerService.Logic.Implementation.Internal.Providers.Configuration.EventLogLoggingProvider"/> is set as the logging provider.
		/// If a custom logging provider is used, this value is ignored.
		/// </summary>
		/// <value></value>
		...
		public string JobStoreType
		{
			get
			{
				string jobStoreType = typeof(RamJobStore).AssemblyQualifiedName;
				if (JobStoreConfigDefinition != null && !string.IsNullOrEmpty(JobStoreConfigDefinition.Type))
				{
					jobStoreType = JobStoreConfigDefinition.Type;
				}
				return jobStoreType;
			}
		}
		...
		public string JobStoreSettingsProviderType
		{
			get
			{
				string jobStoreProviderType = null;
				if (JobStoreConfigDefinition != null)
				{
					jobStoreProviderType = JobStoreConfigDefinition.SettingsProviderType;
				}
